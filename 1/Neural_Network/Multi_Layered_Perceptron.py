...
class perceptron:
    ...
    def __cost_der(self,acti_type,predicted,real):
        if(acti_type==1):
            temp=[]
            for i in range(0,self.num_of_training_examples): temp.append((-real[0][i]/predicted[0][i]+(1-real[0][i])/(1-predicted[0][i]))/self.num_of_training_examples)
    ...
    def train(self):
        ...
        for i in range(self.num_of_iterations):
             ...
             print('iteration '+str(i)+' complete' , 'rmse=',self.rmse,'    ', '('+str(self.rmse-temp)+')'  )
